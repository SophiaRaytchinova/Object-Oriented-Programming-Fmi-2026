...
void VehicleList::resize() {
    ...
    for (unsigned i = 0; i < oldCap; ++i) newData[i] = data[i];
    for (unsigned i = oldCap; i < currentCapacity; ++i) newData[i] = nullptr;
    delete[] data;
    data = newData;
}
...
VehicleList::VehicleList(unsigned cap) : count(0), currentCapacity(cap) {
    data = new Vehicle*[currentCapacity];
    for (unsigned i = 0; i < currentCapacity; ++i) data[i] = nullptr;
}
...
unsigned VehicleList::size() const { return count; }
unsigned VehicleList::capacity() const { return currentCapacity; }
bool VehicleList::isEmpty() const { return count == 0; }
...
VehicleList& VehicleList::operator+=(const Vehicle& v) {
    if (find(v.getRegistration())) return *this;

    if (count == currentCapacity) resize();
...
VehicleList::operator bool() const { return count > 0; }

Vehicle* VehicleList::find(const Registration& r) {
    for (unsigned i = 0; i < currentCapacity; ++i) {
        if (data[i] && data[i]->getRegistration() == r) return data[i];
    }
    return nullptr;
}
...
bool VehicleList::operator>(const VehicleList& other) const {
    ...
    // Лексикографско сравнение на първия намерен номер
    const char* firstThis = nullptr;
    const char* firstOther = nullptr;

    for (unsigned i = 0; i < currentCapacity; ++i) 
        if (data[i]) { firstThis = data[i]->getRegistration().toString(); break; }
    
    for (unsigned i = 0; i < other.currentCapacity; ++i) 
        if (other.data[i]) { firstOther = other.data[i]->getRegistration().toString(); break; }

    if (!firstThis) return false;
    if (!firstOther) return true;
